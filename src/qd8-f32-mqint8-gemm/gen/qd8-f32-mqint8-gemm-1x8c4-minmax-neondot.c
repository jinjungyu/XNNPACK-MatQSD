// MatQSD mqint8 kernel: per-group int8 weight GEMM.
// Based on qd8-f32-qb4w-gemm-1x8c4-minmax-neondot.c with nibble extraction
// replaced by direct int8 loads.
//
// Weight layout (packed by packer):
//   int8 tiled weight data (reconstructed from [upper|lower] nibbles)
//   + bf16 per-group scale + float bias
//
// Dequantize: output = sum_groups( dot(int8_act, int8_weight) * group_scale ) * act_scale
...
void xnn_qd8_f32_mqint8_gemm_minmax_ukernel_1x8c4__neondot(
    size_t mr,
    size_t nc,
    size_t kc,
  ...
  do {
    ...
    for (size_t kb = 0; kb < kc; kb += bl) {
      ...
      while (k >= 8 * sizeof(int8_t)) {
        ...
        // Load a 8x8 block of int8 weights (NOT nibble packed).
        // mqint8: direct int8 load, 2x the data of qb4w per block.
      ...
      if XNN_UNLIKELY(k != 0) {
        ...
        // Load a 4x8 block of int8 weights.
        const int8x16_t vb0123x0123 = vld1q_s8(w); w = (const int8_t*) w + 16;
        const int8x16_t vb0123x4567 = vld1q_s8(w); w = (const int8_t*) w + 16;
      ...
      // Per-group scale (bf16 stored as uint16, convert to float32).
      // NOTE: scale stored is original_scale * 16, kernel divides by 16.
      const float32x4_t vfilter_output_scale0123_raw = vreinterpretq_f32_u32(vshll_n_u16(vld1_u16(w), 16)); w = (const uint16_t*) w + 4;
      const float32x4_t vfilter_output_scale4567_raw = vreinterpretq_f32_u32(vshll_n_u16(vld1_u16(w), 16)); w = (const uint16_t*) w + 4;
      // Divide by 16 to get original scale (scale was stored as original * 16 for 4-bit path).
      const float32x4_t vscale_div16 = vdupq_n_f32(0.0625f);  // 1/16
      const float32x4_t vfilter_output_scale0123 = vmulq_f32(vfilter_output_scale0123_raw, vscale_div16);
      const float32x4_t vfilter_output_scale4567 = vmulq_f32(vfilter_output_scale4567_raw, vscale_div16);
